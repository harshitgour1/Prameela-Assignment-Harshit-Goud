...
export function CompanyTable({
  ...
  const renderSortIcon = (column: string) => {
    if (sortBy !== column)
      return (
        <ArrowUpDown className="ml-1.5 h-3.5 w-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
      );
  ...
  const handleBulkDelete = () => {
    if (
      selectedIds.size > 0 &&
      confirm(`Are you sure you want to delete ${selectedIds.size} selected companies?`)
    ) {
    ...
    return (
      <div className="p-16 text-center flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground max-w-sm">
          There was an issue connecting to the database. Please check your network and try again.
        </p>
        <Button
          variant="outline"
        ...
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong className="text-foreground">{companyToDelete?.name}</strong>? This action
              cannot be undone and will permanently remove this company from the database.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <Table className="w-full border-collapse">
          <TableHeader>
            <TableRow className="border-b-0">
              ...
              <TableHead
                className="font-medium text-muted-foreground text-xs uppercase tracking-wider py-5 group cursor-pointer"
              ...
              <TableHead className="font-medium text-muted-foreground text-xs uppercase tracking-wider py-5">
                Industry
              </TableHead>
              <TableHead className="font-medium text-muted-foreground text-xs uppercase tracking-wider py-5 hidden md:table-cell">
                Website
              </TableHead>
              <TableHead
                className="font-medium text-muted-foreground text-xs uppercase tracking-wider py-5 group cursor-pointer text-center hidden sm:table-cell"
              ...
              <TableHead
                className="font-medium text-muted-foreground text-xs uppercase tracking-wider py-5 group cursor-pointer hidden lg:table-cell"
                onClick={() => onSort('createdAt')}
              >
                <div className="flex items-center">Created At {renderSortIcon('createdAt')}</div>
              </TableHead>
              <TableHead className="text-center font-medium text-muted-foreground text-xs uppercase tracking-wider py-5 w-[100px] hidden sm:table-cell">
                Actions
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody
            className={`transition-opacity duration-200 ${isFetching && !isLoading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
          >
            {isLoading ? (
              ...
              data?.map((company) => {
                ...
                const isDeleting =
                  (deleteMutation.isPending && deleteMutation.variables === company.id) ||
                  ...
                  >
                    ...
                    <TableCell className="py-4">
                      <Popover>
                        ...
                        <PopoverContent
                          className="w-80 max-w-[calc(100vw-2rem)] p-0 overflow-hidden rounded-2xl border-border shadow-xl"
                          sideOffset={8}
                          align="start"
                        >
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center gap-3 text-sm">
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-foreground hover:text-primary truncate flex items-center gap-1"
                                >
                    ...
                    <TableCell className="py-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-md text-[13px] font-medium ${getIndustryBadge(company.industry)}`}
                      >
                        {company.industry}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="py-4 text-center hidden sm:table-cell">
                      <Button
                        ...
                        onClick={() =>
                          setCompanyToDelete({ id: company.id, name: company.companyName })
                        }
                        disabled={isDeleting}
                      >
                        Delete
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-block mt-1 px-2 py-0.5 rounded-md text-[11px] font-medium ${getIndustryBadge(company.industry)}`}
                      >
                        {company.industry}
                      </span>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm mb-4 bg-muted/30 p-3 rounded-lg border border-border/50">
                  <div>
                    <p className="text-muted-foreground text-[11px] uppercase tracking-wider font-semibold mb-0.5">
                      Employees
                    </p>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground flex items-center gap-1.5">
                      ...
                      {format(new Date(company.createdAt), 'MMM d, yy')}
                    </p>
                  </div>
                  <div className="col-span-2 pt-2 mt-1 border-t border-border/50">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-primary hover:text-primary/80 font-medium text-[13px] flex items-center gap-1.5"
                    >
